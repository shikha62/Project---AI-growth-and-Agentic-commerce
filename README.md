...
API routes: app/api/v1/checkout/route.ts, app/api/v1/webhooks/razorpay/route.ts
...
RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET are used server-side to call the Razorpay Orders API. The code checks RAZORPAY_KEY_ID for the rzp_ prefix and will switch to a mocked flow if credentials are missing or invalid. See lib/razorpay.ts.
RAZORPAY_WEBHOOK_SECRET is used to validate incoming webhook signatures in app/api/v1/webhooks/razorpay/route.ts.
...
Webhooks
Incoming Razorpay webhooks are handled at the route implemented in app/api/v1/webhooks/razorpay/route.ts.
